...
public class Scope {
    ...
    public void draw() {
        ...
        double realMinV = 1e8;
        double realMinI = 1e8;
        int curColor = sim.whiteBackground() ? Color.MAGENTA : Color.YELLOW;
        int voltColor = (value > 0) ? 0xFFFFFFFF : (sim.whiteBackground() ? Color.BLUE : Color.GREEN);
        ...
        gridStep = 1e-15;
        double ts = sim.getTimeStep() * speed;
        while (gridStep < ts * 5) {
            gridStep *= 10;
        }
        double tstart = sim.getT() - sim.getTimeStep() * speed * rect.width();
        ...
        if (showFreq) {
            ...
            freq = 1 / (avperiod * sim.getTimeStep() * speed);
        ...
        if (text != null && rect.top + rect.height() > yt + 5) {
            //g.drawString(text, x, yt);
            yt += 15;
        }
        if (showFreq && freq != 0 && rect.top + rect.height() > yt + 5) {
